package com.developer.contas.controller;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.web.bind.annotation.CrossOrigin;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import com.developer.contas.dto.MovimentacaoDTO;
import com.developer.contas.generics.BaseController;
import com.developer.contas.service.MovimentacaoService;

import lombok.RequiredArgsConstructor;

@CrossOrigin(origins = "http://localhost:4200")
@RestController
@RequestMapping("/api/movimentacao")
@RequiredArgsConstructor
public class MovimentacaoController extends BaseController<MovimentacaoDTO,	MovimentacaoService>{

	private static final Logger log = LoggerFactory.getLogger(MovimentacaoController.class);
	
	@GetMapping("/on")
	public String estaOn() {
		log.info("O Serviço de movimentação esta no ar.");
		return "Serviço de movimentação esta no ar.";
	}
}
